...
// Window URL field stripped below and passed to queue to run flask URL routing at @app.route('/geo/<identifier>')
...
queue()
    .defer(d3.json, "/geo/" + lastsegment)
    .await(plotData);

function plotData(error, packetsJson) {

    // Geolocation map uses same setup as overview page although uses requested data from Wigle and mongo based on SSID
