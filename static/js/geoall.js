...
queue()
    .defer(d3.json, "/geoall/" + lastsegment)
    .await(plotData);

function plotData(error, packetsJson) {

    // Geolocation map uses same setup as overview page although uses requested data from Wigle and mongo based on all SSIDs for a device
