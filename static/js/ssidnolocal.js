...
// .js for generating SSID overview page (Excluding local networks)
...
function InputMACBoxNoLocal() {
    var macSearch = document.getElementById("MACLocate2").value;
    if (macSearch.length === 17)
...
queue()
    .defer(d3.json, "/uniquessidsnolocal")
    .await(plotVendor);
...
function plotVendor(error, packetsJson) {
    ...
    var randomisationDim = crossFilter.dimension(function (d) {return d.randomisation;});
    ...
    var randomisationGrp = randomisationDim.group();
    ...
    // Convert to Key Val pair DC group
        ...
        return {
            all: function () {
                ...
                return Object.keys(macNetworkGroup)
                    .map(function (key) {
                        return {key: key, value: macNetworkGroup[key]};
                    });
            },
    ...
    // Question on double reduction asked on StackOverflow method below used
    // https://stackoverflow.com/questions/51677899/dc-js-using-two-reducers-without-a-simple-dimension-and-second-grouping-stage
